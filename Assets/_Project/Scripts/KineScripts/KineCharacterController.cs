...
namespace KinematicCharacterController.Examples
{
    ...
    public class KineCharacterController : MonoBehaviour, ICharacterController
    {
        ...
        [SerializeField] private float interactRange = 3f;       // distanza massima del raycast
        [SerializeField] private LayerMask interactLayers;       // layer degli oggetti interagibili
        [SerializeField] private GameObject interactIcon;        // icona UI
        private Interactable currentTarget;                      // riferimento all’oggetto attuale
        ...
        private void Awake()
        {
            ...
            //Grandezze direttamente dal collider
            ...
            if (MeshRoot != null)
            {
                originalMeshLocalScale = MeshRoot.localScale;
            }
            if (CameraFollowPoint != null)
            {
                defaultCameraLocalPos = CameraFollowPoint.localPosition;
            }
                ...
                case CharacterState.Default:
                    {
                        ...
                        if (inputs.CrouchDown)
                        {
                            ...
                            if (!_isCrouching)
                            {
                                ...
                                if (MeshRoot != null)
                                {
                                    MeshRoot.localScale = new Vector3(originalMeshLocalScale.x, originalMeshLocalScale.y * 0.5f, originalMeshLocalScale.z);
                                }
                                /*if (CameraFollowPoint != null)
                                {
                                    CameraFollowPoint.localPosition = defaultCameraLocalPos - new Vector3(0f, _defaultCapsuleHeight * 0.5f, 0f);
                                }*/
                            }
                        }
                        else if (inputs.CrouchUp)
                        {
                            ...
                            /*if (CameraFollowPoint != null)
                            {
                                CameraFollowPoint.localPosition = defaultCameraLocalPos;
                            }*/
                        }
                        ...
                        if (inventory != null)
                        {
                            ...
                            if (inputs.Item1)
                            {
                                Debug.Log("Item1 click");
                                inventory.EquipItem(0);
                            }
                            else if (inputs.Item2)
                            {
                                Debug.Log("Item2 click");
                                inventory.EquipItem(1);
                            }
                            else if (inputs.Item3)
                            {
                                Debug.Log("Item3 click");
                ...
                case CharacterState.Default:
                    {
                        ...
                        if (Motor.GroundingStatus.IsStableOnGround)
                        {
                            ...
                            //calcoli per i casi di salto e crouch
                            float currentMaxSpeed = MaxStableMoveSpeed;

                            // Se crouching rallenta
                            if (_isCrouching)
                            {
                                currentMaxSpeed *= CrouchSpeedMultiplier;
                            }
                            // Se sprint accelera (solo se non stai saltando o crouchando)
                            else if (!_isCrouching && Motor.GroundingStatus.IsStableOnGround && _moveInputVector.sqrMagnitude > 0f && _sprinting)
                            {
                                currentMaxSpeed *= SprintSpeedMultiplier;
                            }

                            // Calcola la velocità target con il moltiplicatore
                ...
                case CharacterState.Default:
                    {
                        ...
                        if (_isCrouching && !_shouldBeCrouching)
                        {
                            ...
                            //MEZZ'ORA PERSA PER STO RIGO FIGLIO DI BRAVA DONNA, CHE IL CLOWN CHE SONO HA PURE RILETTO PIÙ VOLTE ZIO PERA
        ...
        private void TryInteract()
        {
            ...
            //Ray ray = new Ray(CameraFollowPoint.position, CameraFollowPoint.forward);
            Ray ray = Camera.main.ViewportPointToRay(new Vector3(0.5f, 0.5f, 0.5f));
            if (Physics.Raycast(ray, out RaycastHit hit, interactRange, interactLayers))
            {
                Debug.Log($"raycast hit {hit.collider.name}");
                // prima controlla se è un ItemObject
                ...
                // altrimenti, se è un Interactable generico (tipo leva, NPC, ecc.)
        ...
        {
            if (CameraFollowPoint == null || interactIcon == null) return;
            ...
            if (Physics.Raycast(ray, out RaycastHit hit, interactRange, interactLayers))
            {
                ...
                // Oppure un Interactable generico
                else if (hit.collider.TryGetComponent<Interactable>(out var interactable))
                {
            ...
            interactIcon.SetActive(foundSomething);
        }
